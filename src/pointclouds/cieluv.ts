...
export class CIELUVPointCloud extends PointCloud {
    ...
    private ppmBuffer: GPUBuffer | undefined;

    private luvBGL = WGPU_RENDERER.device.createBindGroupLayout({
        entries: [
            {
                binding: 0,
    ...
    private luvBindGroup: GPUBindGroup | null = null;
    private luvPipeline: GPUComputePipeline | null = null;

    // PPM file will contain RGB values for each pixel
    private rgbToLUVKernel = `
    ...
    constructor(gridSize: number) {
        ...
        // Generate LUV point cloud from scratch
            ...
            this.luvPipeline = this.createComputePipeline(
                this.luvKernel,
                [this.transformBGLayout],
                "LUV Kernel",
            );
