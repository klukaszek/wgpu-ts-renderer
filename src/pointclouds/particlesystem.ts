...
export class ParticleSystem extends PointCloud {
    ...
    private computeBindgroups: [GPUBindGroup, GPUBindGroup] = [null as any, null as any];

    constructor(numParticles: number, transform?: Transform) {
    ...
    public override render(renderPass: GPURenderPassEncoder): void {
    ...
    public setForce(x: number, y: number, z: number): void {
        ...
        WGPU_RENDERER.device.queue.writeBuffer(
            this.simParamsBuffer,
            24, // Offset to forces in SimParams
            forceData
        );
    }
    ...
    public setParticleLife(life: number): void {
    ...
    public setEmitterPosition(position: vec3): void {
